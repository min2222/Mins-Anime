...
public class EntityAltair extends AbstractAnimatableMonster implements IClipPos
{
    ...
    public void tick() 
    {
    	...
    	if(this.getTarget() != null)
    	{
    		...
    		{
    			Vec3 motion = AnimeUtil.fromToVector(AnimeUtil.getSpreadPosition(this.level, this.position(), 8), this.getTarget().position(), 0.5F);
    			this.setDeltaMovement(this.getDeltaMovement().add(motion));
    		}
    		if(this.distanceTo(this.getTarget()) <= 8.0F)
    		{
    			Vec3 motion = AnimeUtil.fromToVector(this.getTarget().position(), AnimeUtil.getSpreadPosition(this.level, this.position(), 8), 0.5F);
    			this.setDeltaMovement(this.getDeltaMovement().add(motion));
    		}
    ...
    {
    	if(!p_21016_.is(DamageTypeTags.BYPASSES_INVULNERABILITY) && this.shouldClip())
    	{
