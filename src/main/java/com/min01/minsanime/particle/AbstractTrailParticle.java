...
//https://github.com/AlexModGuy/AlexsCaves/blob/main/src/main/java/com/github/alexmodguy/alexscaves/client/particle/AbstractTrailParticle.java
public abstract class AbstractTrailParticle extends Particle
{
    ...
    public void render(VertexConsumer consumer, Camera camera, float partialTick)
    {
        if(this.trailPointer > -1)
        {
            ...
            Vec3 topAngleVec = new Vec3(0, this.getTrailHeight() / 2.0F, 0).zRot(zRot);
            Vec3 bottomAngleVec = new Vec3(0, this.getTrailHeight() / -2.0F, 0).zRot(zRot);
            ...
            while(samples < this.sampleCount()) 
            {
                ...
                float u1 = samples / (float) this.sampleCount();
                float u2 = u1 + 1 / (float) this.sampleCount();
                ...
                vertexconsumer.vertex(matrix4f, (float) draw1.x + (float) bottomAngleVec.x, (float) draw1.y + (float) bottomAngleVec.y, (float) draw1.z + (float) bottomAngleVec.z).color(this.trailR, this.trailG, this.trailB, this.trailA).uv(u1, 1.0F).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(j).normal(matrix3f, 0.0F, 1.0F, 0.0F).endVertex();
                vertexconsumer.vertex(matrix4f, (float) draw2.x + (float) bottomAngleVec.x, (float) draw2.y + (float) bottomAngleVec.y, (float) draw2.z + (float) bottomAngleVec.z).color(this.trailR, this.trailG, this.trailB, this.trailA).uv(u2, 1.0F).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(j).normal(matrix3f, 0.0F, 1.0F, 0.0F).endVertex();
                vertexconsumer.vertex(matrix4f, (float) draw2.x + (float) topAngleVec.x, (float) draw2.y + (float) topAngleVec.y, (float) draw2.z + (float) topAngleVec.z).color(this.trailR, this.trailG, this.trailB, this.trailA).uv(u2, 0).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(j).normal(matrix3f, 0.0F, 1.0F, 0.0F).endVertex();
                vertexconsumer.vertex(matrix4f, (float) draw1.x + (float) topAngleVec.x, (float) draw1.y + (float) topAngleVec.y, (float) draw1.z + (float) topAngleVec.z).color(this.trailR, this.trailG, this.trailB, this.trailA).uv(u1, 0).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(j).normal(matrix3f, 0.0F, 1.0F, 0.0F).endVertex();
                samples++;
                drawFrom = sample;
            }
